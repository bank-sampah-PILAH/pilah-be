...
gcloud run deploy "${SERVICE_NAME}" \
  ...
  --set-env-vars="^|^DJANGO_DEBUG=false|DJANGO_ALLOWED_HOSTS=*|CORS_ALLOW_ALL_ORIGINS=true|DB_ENGINE=django.db.backends.postgresql|DB_NAME=${DB_NAME}|DB_USER=${DB_USER}|DB_PASSWORD=${DB_PASSWORD}|DB_HOST=/cloudsql/${SQL_CONNECTION_NAME}|DB_PORT=5432|GS_BUCKET_NAME=${GCS_BUCKET}|PILAH_ALLOW_FAKE_GOOGLE_TOKEN=false|PILAH_PUBLIC_APP_URL=${PILAH_PUBLIC_APP_URL}|GOOGLE_CLIENT_ID=${GOOGLE_CLIENT_ID}|GOOGLE_CLIENT_IDS=${GOOGLE_CLIENT_IDS}|WHATSAPP_GATEWAY_URL=${WHATSAPP_GATEWAY_URL}|WHATSAPP_GATEWAY_TOKEN=${WHATSAPP_GATEWAY_TOKEN}|DJANGO_SECRET_KEY=${DJANGO_SECRET_KEY}"
